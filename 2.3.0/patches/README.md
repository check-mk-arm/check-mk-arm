...
| `0300-check_mk-idempotent-check-rename` | The legacy-check `.py`-stripping rename moves files onto themselves the second time it runs. One of the target's prerequisites is `.PHONY`, so it runs on *every* invocation — meaning every resumed build fails here. Not architecture-specific; upstream never hits it because their builds always start clean. | upstream restricts the find, or drops the .PHONY prerequisite |
